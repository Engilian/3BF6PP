    ...
    m_smooth(false)
{
  ...
  //    m_graph->setBarSpacing(QSizeF(100,50));
  ...
  m_temperatureAxis->setLabelFormat(QString(tr("%.1f ") + celsiusString));
  m_temperatureAxis->setLabelAutoRotation(30.0f);
  m_temperatureAxis->setTitleVisible(false);
  m_temperatureAxis->setAutoAdjustRange(true);

  m_yearAxis->setTitle("Year");
  m_yearAxis->setLabelAutoRotation(30.0f);
  m_yearAxis->setTitleVisible(true);
  m_yearAxis->setAutoAdjustRange(true);
  m_monthAxis->setTitle("Month");
  m_monthAxis->setLabelAutoRotation(30.0f);
  m_monthAxis->setTitleVisible(true);
  m_monthAxis->setAutoAdjustRange(true);
  ...
  //! [8]
  m_primarySeries->setItemLabelFormat(tr("@colLabel @rowLabel: @valueLabel"));
  m_primarySeries->setMesh(QAbstract3DSeries::MeshBevelBar);
  m_primarySeries->setMeshSmooth(false);

  m_secondarySeries->setItemLabelFormat(tr("@colLabel @rowLabel: @valueLabel"));
  ...
  camera->setCameraPosition(1000,0);
...
void GraphModifier::setData(QList<GraphItem> data)
{
  if (!data.isEmpty())
  {
    m_years.clear();
    ...
    QSet<QString> allMonth;
    for (auto v : data){
      m_years.append(v.rowTitle);
      for (auto c : v.columns) {
        allMonth.insert(c.caption);
      }
    }

    auto mountList = allMonth.toList();
    std::sort( mountList.begin(), mountList.end(), [](const QString &s1, const QString &s2) {
      return s1 < s2;
    });

    //    for (auto v : data[0].columns)
    //      m_months.append(v.caption);

    for (auto m : mountList )
      m_months.append(m);
  }
  ...
  for (int year = 0; year < m_years.size(); year++) {
    ...
    for (int month = 0; month < m_months.size(); month++) {
      // Add data to the row
      (*dataRow)[month].setValue(data[year].columns[month].value.toFloat());
    }
  ...
  m_yearAxis->setLabels({});
  m_monthAxis->setLabels({});

  changeRange( 0 );
  changeRangeByY( 0 );
}

void GraphModifier::setMinMax(float min, float max)
{
  m_minval = min;
  m_maxval = max;
  m_temperatureAxis->setRange(m_minval, m_maxval);
...
void GraphModifier::setTitleByX(QString title)
{
  m_yearAxis->setTitle(title);

}

void GraphModifier::setTitleByY(QString title)
{
  m_monthAxis->setTitle(title);
}

void GraphModifier::setTitleByZ(QString title)
{
  m_temperatureAxis->setLabelFormat(QString(tr("%.1f ") + title));
}
...
void GraphModifier::changeRange(int range)
{
  ...
  if (range >= m_months.count())
    m_monthAxis->setRange(0, m_months.count() - 1);
  else
    m_monthAxis->setRange(range, range);
}

void GraphModifier::changeRangeByY(int range)
{
  if (range >= m_years.count())
    m_yearAxis->setRange(0, m_years.count() - 1);
  else
  ...
  if (range<0)
    range=0;
  if ((range<data_.count())) {
    auto elem =   data_[range];
    auto max= elem.max();
    setMinMax(0,max);
  }

  //    if ( !data_.isEmpty() ) {
  //      auto elem = std::max( data_.begin(), data_.end() );
  //      setMinMax( 0, elem->max() );
  //    } else {
  //      setMinMax( 0, 0 );
  //    }
}
